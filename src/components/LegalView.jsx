...
import { ShieldCheck, FileText, Info, Mail, ArrowLeft } from 'lucide-react';

export default function LegalView({ type, onBackHome, lang = 'en' }) {
  const isEn = lang === 'en';
  ...
  return (
    <div style={{ maxWidth: '960px', margin: '0 auto', padding: '32px 16px' }}>
      ...
      >
        <ArrowLeft size={16} />
        <span>{isEn ? '← Back to Home' : '← Regresar al Inicio'}</span>
      </button>
        ...
        {pageType === 'privacy' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px', paddingBottom: '20px', borderBottom: '1px solid var(--border)' }}>
              <div style={{ padding: '12px', backgroundColor: 'var(--primary-soft)', color: 'var(--primary)', borderRadius: '12px' }}>
                <ShieldCheck size={32} />
              </div>
              <div>
                ...
                <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                  {isEn ? 'Official GDPR & CCPA Compliance' : 'Cumplimiento Oficial RGPD y CCPA'} — findcalculator.online
                </span>
              </div>
            </div>

            <div style={{ fontSize: '1rem', lineHeight: 1.8, color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <p>
                {isEn
                  ...
                  : 'En FindCalculator (https://findcalculator.online), la privacidad de nuestros visitantes es de máxima prioridad. Este documento de Política de Privacidad detalla la información recopilada y cómo es utilizada por nuestra plataforma.'}
              </p>

              <h2 style={{ color: 'var(--text-primary)', fontSize: '1.25rem', fontWeight: 700, marginTop: '12px' }}>
                {isEn ? '1. Log Files & Visitor Analytics' : '1. Archivos de Registro y Analítica de Visitantes'}
              </h2>
              <p>
                {isEn
              ...
              <p>
                {isEn
                  ? 'Google is one of a third-party vendor on our site. It also uses cookies, known as DART cookies, to serve ads to our site visitors based upon their visit to findcalculator.online and other sites on the internet. However, visitors may choose to decline the use of DART cookies by visiting the Google ad and content network Privacy Policy at the following URL – https://policies.google.com/technologies/ads'
                  : 'Google es un proveedor de terceros en nuestro sitio web. Utiliza cookies, como la cookie de DART, para publicar anuncios a los visitantes en función de sus visitas a findcalculator.online y otros sitios web en internet. Los usuarios pueden declinar el uso de la cookie de DART visitando la Política de Privacidad de anuncios de Google en: https://policies.google.com/technologies/ads'}
              </p>

              <h2 style={{ color: 'var(--text-primary)', fontSize: '1.25rem', fontWeight: 700, marginTop: '12px' }}>
                {isEn ? '3. Local Storage Privacy & Data Security' : '3. Privacidad y Almacenamiento Local (LocalStorage)'}
              </h2>
              <p>
                {isEn
              ...
              <h2 style={{ color: 'var(--text-primary)', fontSize: '1.25rem', fontWeight: 700, marginTop: '12px' }}>
                {isEn ? '4. GDPR & CCPA User Rights' : '4. Derechos RGPD y CCPA de los Usuarios'}
              </h2>
              <p>
                {isEn
              ...
              <h2 style={{ color: 'var(--text-primary)', fontSize: '1.25rem', fontWeight: 700, marginTop: '12px' }}>
                {isEn ? '5. Contact Information' : '5. Información de Contacto Oficial'}
              </h2>
              <p>
                {isEn
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px', paddingBottom: '20px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div>
                ...
                <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>findcalculator.online</span>
              </div>
            </div>
            ...
            <div style={{ fontSize: '1rem', lineHeight: 1.8, color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ backgroundColor: 'var(--surface-alt)', padding: '24px', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontWeight: 700, color: 'var(--text-primary)', fontSize: '1.0625rem' }}>
                  <Mail size={22} color="var(--primary)" />
                  <span>{isEn ? 'Official Support & Contact Information:' : 'Información Oficial de Soporte y Contacto:'}</span>
                </div>
